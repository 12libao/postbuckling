  ...
  Kokkos::parallel_for(
      nelems, KOKKOS_LAMBDA(const int n) {
        for (int i = 0; i < 4; ++i) {
          ...
          J(n, 1, 0) += Nxi(i) * ye(n, i);
          J(n, 0, 1) += Neta(i) * xe(n, i);
        ...
        for (int i = 0; i < 4; ++i) {
          Be(n, 0, i * 2) = invJ(n, 0, 0) * Nxi(i) + invJ(n, 1, 0) * Neta(i);
          Be(n, 1, i * 2 + 1) = invJ(n, 0, 1) * Nxi(i) + invJ(n, 1, 1) * Neta(i);
...
// const int nnodes = std::max_element(conn.data(), conn.data() + conn.size())[0] + 1;
...
// // Ke.flatten()
// View1D<D> Ke_flat("Ke_flat", nelems * 64);
// Kokkos::parallel_for(
//     nelems, KOKKOS_LAMBDA(const int n) {
//       for (int ii = 0; ii < 8; ii++) {
//         for (int jj = 0; jj < 8; jj++) {
...
// tick("crs");
// // after get the i_index, j_index, Ke_flat, we can use them to construct the sparse matrix
// // using the KokkosSparse::CrsMatrix
// auto crsMat = KokkosSparse::coo2crs(2 * nnodes, 2 * nnodes, i_index, j_index, Ke_flat);
// tock("crs");

// // return the data, indices, indptr to the python
// auto data_view = crsMat.values;
// auto indices_view = crsMat.graph.entries;
// auto indptr_view = crsMat.graph.row_map;

// tick("copy");
// Kokkos::deep_copy(data, data_view);
// Kokkos::deep_copy(indices, indices_view);
// Kokkos::deep_copy(indptr, indptr_view);
// tock("copy");