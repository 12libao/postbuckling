...
View1D<T> numpyArrayToView1D(const py::array_t<T, py::array::c_style>& array) {
  ...
  view = Kokkos::View<T*, ExecSpace>(data_ptr, info.shape[0]);
...
View2D<T> numpyArrayToView2D(const py::array_t<T, py::array::c_style>& array) {
  ...
  view = Kokkos::View<T**, ExecSpace>(data_ptr, info.shape[0], info.shape[1]);
#endif
...
View3D<T> numpyArrayToView3D(const py::array_t<T, py::array::c_style>& array) {
  ...
  view = Kokkos::View<T***, ExecSpace>(data_ptr, info.shape[0], info.shape[1], info.shape[2]);
#endif
...
py::array_t<T, py::array::c_style> viewToNumpyArray1D(const View1D<T>& view) {
  ...
  std::vector<ssize_t> shape_vec = {static_cast<ssize_t>(shape)};
  // Create numpy array with shared memory from Kokkos view
  py::array_t<T, py::array::c_style> result(shape_vec, view.data());
  return result;
}
template <typename T>
py::array_t<T, py::array::c_style> viewToNumpyArray2D(const View2D<T>& view) {
  auto shape = view.extent(0);
  auto shape1 = view.extent(1);
  // Create numpy array with shared memory from Kokkos view
  py::array_t<T, py::array::c_style> result({shape, shape1}, view.data());
  return result;
}
...
py::array_t<T, py::array::c_style> viewToNumpyArray3D(const View3D<T>& view) {
  auto shape = view.extent(0);
  auto shape1 = view.extent(1);
  auto shape2 = view.extent(2);
  // Create numpy array with shared memory from Kokkos view
  py::array_t<T, py::array::c_style> result({shape, shape1, shape2}, view.data());
  return result;
}
...
py::array_t<T, py::array::c_style> viewToNumpyArray4D(const View4D<T>& view) {
  ...
  auto shape1 = view.extent(1);
  auto shape2 = view.extent(2);
  auto shape3 = view.extent(3);
  // Create numpy array with shared memory from Kokkos view
  py::array_t<T, py::array::c_style> result({shape, shape1, shape2, shape3}, view.data());
  return result;
}
...
py::array_t<T, py::array::c_style> viewToNumpyArray5D(const View5D<T>& view) {
  ...
  auto shape2 = view.extent(2);
  auto shape3 = view.extent(3);
  auto shape4 = view.extent(4);
  // Create numpy array with shared memory from Kokkos view
  py::array_t<T, py::array::c_style> result({shape, shape1, shape2, shape3, shape4}, view.data());
  return result;
}
...
py::array_t<T, py::array::c_style> viewToNumpyArray6D(const View6D<T>& view) {
  ...
  auto shape3 = view.extent(3);
  auto shape4 = view.extent(4);
  auto shape5 = view.extent(5);
  // Create numpy array with shared memory from Kokkos view
  py::array_t<T, py::array::c_style> result({shape, shape1, shape2, shape3, shape4, shape5},
                                            view.data());
  return result;
}
...
//   // py::array_t<T, py::array::c_style> result({shape});
...
//   py::array_t<T, py::array::c_style> result({shape, shape1});
//   py::buffer_info buf = result.request();
//   T* ptr = static_cast<T*>(buf.ptr);
...
//   py::array_t<T, py::array::c_style> result({shape, shape1, shape2});
...
//   py::array_t<T, py::array::c_style> result({shape, shape1, shape2, shape3});
//   py::buffer_info buf = result.request();
//   T* ptr = static_cast<T*>(buf.ptr);
...
//   py::array_t<T, py::array::c_style> result({shape, shape1, shape2, shape3, shape4});
//   py::buffer_info buf = result.request();
//   T* ptr = static_cast<T*>(buf.ptr);
