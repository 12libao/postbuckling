...
def lanczos_adjoint(
    ...
    """
    Compute the Lanczos approximation of the eigenvector adjoint equations.
...
def gmres(
    ...
    """
    Use right-preconditioned GMRES to solve the eigenvector adjoint system.
...
def shift_invert(
    ...
    and invert Krylov subspace method.
...
class BasicLanczos:
    ...
    def solve_adjoint(
        ...
        **kwargs,
        ...
        ----------------
        method : str
            The type of method to use either "pcpg", "gmres", "shift-invert", or "approx-lanczos"
        psi : ndarray
        ...
        if method == "ad-adjoint":
            lanczos_guess = False

        data = {}  # The adjoint correction data
        if lanczos_guess or method == "approx-lanczos":
            psi = lanczos_adjoint(
                Phib,
                self.B,
                self.factor,
