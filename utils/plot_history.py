...
def plot_history(filename, savename=None):
    ...
    if len(values[0]) > 0:
        ...
        if len(values[0]) > 0:
            ...
            ax2 = ax1.twinx()
            l2 = ax2.semilogy(
                iteration, opt_linfty, color=colors[1], label="opt-linfty"
            )
            ...
            l3 = ax2.semilogy(iteration, linfty, color=colors[3], label="linf-opt")
            l4 = ax2.semilogy(iteration, lambd, color=colors[2], label="l1-lambda")
    ...
    if savename is not None:
        fname = os.path.splitext(savename)[0]  # Delete suffix
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "filename", metavar="paropt.out", type=str, help="ParOpt output file name"
    )
    p.add_argument("--savename", type=str, help="Save the plot as a png")
    args = p.parse_args()

    # call plot_history
    plot_history(args.filename, args.savename)
